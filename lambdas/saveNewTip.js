...
export const handler = async (event) => {
    ...
    const tip = JSON.parse(event.tip_str);
    const uuid_array = JSON.parse(event.uuid_array);

    // Call prexisting saveTips lambda
    const updatedUserStr = await saveTip(anonymousId, item_uuid, tip); 
...
const saveTip = async (anonymousId, item_uuid, tip) => {
    const lambdaParams = {
